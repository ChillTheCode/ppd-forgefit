...
const RevisiInputStokBarang = () => {
  ...
  const handleSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const item = barang.find((item) => item.kodeBarang === e.target.value);
  ...
  const handleAddRow = () => {
    ...
    setRows([...rows, { ...selectedItem, stokBarang: selectedItem.stokBarang.toString(), inputStok: parseInt(inputStok) }]);
    setInputStok("");
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (idPengajuan === undefined) {
        navigate("/unauthorized");
        return;
      }

      const response = await pengadaanpusat.revisiPengajuan(
        accessToken,
        idPengajuan,
        stockData
      );
...
export default RevisiInputStokBarang;