...
const PersetujuanKepalaCKS = () => {
  ...
  useEffect(() => {
    ...
    const fetchDataAndBarang = async () => {
      ...
      if (fetchedNomorCabang) {
         ...
         try {
            const response = await pengadaanCKSservice.getBarang(fetchedNomorCabang);
            if (response.status !== 200) {
  ...
  const handleSetuju = async () => {
    try {
      ...
      const response = await pengadaanCKSservice.pengecekanKepalaCabang(
        accessToken,
        idPengajuan,
        true,
        nomorCabang,
      ...
      if (response.data === true) {
        ...
        navigate(`/pengadaan-cabang-kerja-sama/cabang/${nomorCabang}`);
      }
    } catch (error) {
    ...
    <div className="flex flex-col pb-10">
      <h1 className="py-5 ps-5">
        {roleUser === "Kepala Operasional Cabang" 
          ? "Halaman Persetujuan Pengadaan Cabang Kerja Sama"
          : "Informasi Pengajuan"}
      </h1>
      <DataTable
        value={products}
...
export default PersetujuanKepalaCKS;
